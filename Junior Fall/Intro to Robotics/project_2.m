...
for n = 1:(params.segments)
    ...
    for t = t0:0.1:(t0+params.segment_time)
        ...
        theta = evaluateCubicSpline(theta_0, theta_f, t0, t0+params.segment_time, t);
...
figure(2); hold on
plot(t_trajectory,wrapToPi(theta_trajectory),'.','MarkerSize',10)
plot(t_waypoints,wrapToPi(theta_waypoints),'o','MarkerSize',10)
xlabel('time (s)'); ylabel('joint angles (rad)')
legend('\theta_1','\theta_2','\theta_3','\theta_4','\theta_5',...
    '\theta_6','\theta_7','Orientation','horizontal')
yticks([-4/4*pi -3/4*pi -2/4*pi -1/4*pi 0 1/4*pi 2/4*pi 3/4*pi 4/4*pi])
yticklabels({'-\pi' '-3\pi/4' '-\pi/2' '-\pi/4' '0' '\pi/4' '\pi/2' '3\pi/4' '\pi'})
set(gca,'FontSize',22)

% TODO: Plot end-effector trajectory and waypoints
figure(3); hold on
plot(t_trajectory,gripper_trajectory,'.','MarkerSize',10)
plot(t_waypoints,gripper_waypoints(:,1),'bo','MarkerSize',10)
plot(t_waypoints,gripper_waypoints(:,2),'ro','MarkerSize',10)
plot(t_waypoints,gripper_waypoints(:,3),'yo','MarkerSize',10)
xlabel('time (s)'); ylabel('end effector position (m)')
...
plot(t_history(1:642),wrapToPi(theta_history(1:642,:)),'-')
plot(t_history(643:end),wrapToPi(theta_history(643:end,:)),'-')
plot(t_history(1:590),wrapToPi(theta_des_history(1:590,:)),'--')
plot(t_history(591:end),wrapToPi(theta_des_history(591:end,:)),'--')
xlabel('time (s)'); ylabel('\theta and \theta_d (rad)')
legend('\theta_1','\theta_2','\theta_3','\theta_4','\theta_5',...
    '\theta_6','\theta_7','\theta_{d1}','\theta_{d2}','\theta_{d3}',...
    '\theta_{d4}','\theta_{d5}','\theta_{d6}','\theta_{d7}',...
...
%         t         time of interest
%
% Outputs: theta    1x7 vector for configuration at time t
%
function [theta] = evaluateCubicSpline(theta0, thetaf, t0, tf, t)
...
function [tau,theta_des]  = controlLaw(robot, t, theta, theta_dot,params)
    %% TODO: Uncomment
    [theta_des] = getThetaDesired(t, params);
...
%
% Outputs: p    a 1x3 vector for the position of the n-th waypoint
