...
xy = 0.3+0.5.*sind(theta);          % y component of spring lenght
x = sqrt(xx.^2 + xy.^2);            % spring length at different thetas
    
%% FOR STATEMENT
% Create cases where theta <,=,> thetaUn
% Determine the spring force
% Determine moment about O
for i = 1:1:N    
    %if theta(i) < thetaUn || theta(i) > (180-thetaUn)
...
%     elseif theta(i) == thetaUn
%         Fa = 0;                                     % Spring Force
%     else
%         FaMag(i) = k.*(x(i)-xUn);                   % Spring Force magnitude
%         BwrtA = (-xx(i)*ihat) + (-xy(i)*jhat);      % Vector from A to B
%         BwrtAhat = BwrtA / x(i);                    % Unit Vector A to B
%         Fa = FaMag(i)*BwrtAhat;                     % Spring Force
%     end
